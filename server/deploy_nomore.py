...
# Final Space Credentials (Nomore)
token = "os.getenv("HF_TOKEN")"
repo_id = "monldds/nomore" 

api = HfApi(token=token)

print(f"--- Deploying to User Space: {repo_id} ---")
try:
...
if os.path.exists(dockerfile_path):
    ...
    lines.append(f"\n# BUILD_TRIGGER_NOMORE: {time.time()}\n")
    
    with open(dockerfile_path, "w") as f:
        f.writelines(lines)
    print("Added Build Trigger for 'nomore' to Dockerfile.")

print("Uploading files to new space...")
