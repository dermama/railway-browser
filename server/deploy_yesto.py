...
# Final Space Credentials (Yesto)
token = "os.getenv("HF_TOKEN")"
repo_id = "monakoki/yesto" 

api = HfApi(token=token)

print(f"--- Deploying to New Account Space: {repo_id} ---")
try:
    # Ensure it's treated as a Docker Space
    create_repo(repo_id=repo_id, repo_type="space", space_sdk="docker", exist_ok=True)
    print("Repository is ready.")
except Exception as e:
    print(f"Note: Repository creation step (exist_ok=True) returned: {e}")
...
if os.path.exists(dockerfile_path):
    ...
    lines.append(f"\n# BUILD_TRIGGER_YESTO: {time.time()}\n")
    
    with open(dockerfile_path, "w") as f:
        f.writelines(lines)
    print("Added Build Trigger for 'yesto' to Dockerfile.")

print("Uploading files to new account space...")
